...
def typeOfHand(hand):
	'''Returns what is in one's hand'''
	# A hand looks like: ['AS', 'KD', '3D', 'JD', '8H']
	translation = {'2':0,'3':1,'4':2,'5':3,'6':4,'7':5,'8':6,'9':7,'T':8,'J':9,'Q':10,'K':11,'A':12}
	# Royal Flush: Ten, Jack, Queen, King, Ace, in same suit.
	face = [translation[card[0]] for card in hand]
	suits = [card[1] for card in hand]
	if set(face) == {8,9,10,11,12} and len(set(suits)) == 1: 
		return "Royal Flush"
	# Straight Flush: All cards are consecutive values of same suit.
	if set(face) in [{i + j for i in range(1,6)} for j in range(1,8)] and len(set(suits)) == 1:
	...
	# Here, if we only had 2 faces, and it is not four of a kind, it is a full house
	if len(set(face)) == 2:
		return "Full house"
	...
	if set(face) in [{i + j for i in range(1,6)} for j in range(1,8)]:
		return "Straight"
...
def win(hand1,hand2):
	'''Returns True if Hand 1 wins'''
	translationHand = {"Royal Flush":9,"Straight Flush":8,"Four of a Kind":7,"Full house":6,"Flush":5,"Straight":4,"Three of a Kind":3,"Two Pairs":2,"Flush":1,"High Card":0}
	translation = {'2':0,'3':1,'4':2,'5':3,'6':4,'7':5,'8':6,'9':7,'T':8,'J':9,'Q':10,'K':11,'A':12}
	# Royal Flush: Ten, Jack, Queen, King, Ace, in same suit.
	faces1 = [translation[card[0]] for card in hand1]
	suits1 = [card[1] for card in hand2]
	faces2 = [translation[card[0]] for card in hand2]
	suits2 = [card[1] for card in hand2]
	type1 = typeOfHand(hand1)
	type2 = typeOfHand(hand2)
	if translationHand[typeOfHand(hand1)] > translationHand[typeOfHand(hand2)]:
		return True
	if type1 == "High Card":
		if max(faces1) > max(faces2):
			return True
		else:
			print("Deal with high card match")
	if type2 == "High Card":
		if max(faces1) > max(faces2):
			return True
		else:
...
2H 2D 4C 4D 4S 3C 3D 3S 9S 9D'''
	matches = []
	for l in data.split('\n'):
		p1Hand, p2Hand = l.split(' ')[0:5],l.split(' ')[5:10]
		matches.append((p1Hand,p2Hand))
	count = 0
	for match in matches[0:10]:
		print(match,typeOfHand(match[0]),typeOfHand(match[1]))
		#if win(*match):
		#	count += 1
	return count
