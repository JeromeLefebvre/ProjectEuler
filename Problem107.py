    ...
    for r in range(0, 7):
        for c in range(r, 7):
            ...
            if samplematrix[r][c] != '-':
                ...
                samplenetwork[(indices[r], indices[c])] = int(
                    samplematrix[r][c])
...
def problem107():
    ...
    SamepleData = '''-,-,-,427,668,495,377,678,-,177,-,-,870,-,869,624,300,609,131,-,251,-,-,-,856,221,514,-,591,762,182,56,-,884,412,273,636,-,-,774
...
774,289,833,257,-,381,239,722,711,468,933,-,-,17,-,-,148,-,-,853,-,-,-,-,264,194,260,947,-,752,147,-,-,343,112,273,344,680,540,-'''
    ...
    for r, row in enumerate(SamepleData.split('\n')):
        for c, col in enumerate(row.split(',')):
            if col != '-':
                G.add_edge(r, c, weight=int(col))
    H = nx.minimum_spanning_tree(G)

    total_G = 0
    for v in G.edges_iter():
        total_G += G[v[0]][v[1]]['weight']

    total_H = 0
    for v in H.edges_iter():
        total_H += G[v[0]][v[1]]['weight']
